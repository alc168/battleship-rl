...
The computer opponent loads `public/ai_policy.json`, a lookup table generated by the PyTorch DQN trainer in `alc168/battleship-rl`.

- For every board state it has seen during training, the JSON contains the Teacher's top recommended `[row, col]` shot.
- If the current board state is not in the policy, or `ai_policy.json` is not present, the computer falls back to hunt/adjacent logic and then random fire.
- Replace `public/ai_policy.json` with your trained policy from the Colab run to make the AI stronger.

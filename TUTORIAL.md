...
**A useful reading order:** start with `README.md`, then `docs/ARCHITECTURE.md` and `docs/HISTORY.md`. After that, dive into `web/src/App.jsx`, `web/src/utils.js`, `web/src/training.worker.js`, and `worker/index.js`. Should you read `ai_policy.json` from cover to cover, please seek help.
